...
For verifying the **deployed backend** (including real Redis/Postgres connections inside Docker), we use a specialized Pytest suite:

* **File**: `backend/tests/integration/test_live_api.py`
* **Purpose**: Verify critical paths (Auth, Jobs, Cache, Metrics) against the running container.
* **Command**:

    ```bash
    docker exec headhunter_backend python -m pytest tests/integration/test_live_api.py
    ```

* **Key Difference**: Unlike standard integration tests (SQLite), this hits `http://localhost:30001` to test the actual deployed service.

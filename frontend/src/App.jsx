...
import { 
  ...
  ChevronRight, Save, AlertCircle, Check
} from 'lucide-react'
...
const formatCurrency = (val) => {
  if (!val) return ''
  return new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(val).replace('USD', '').trim()
}
    ...
    <div className="flex h-screen bg-[#F8FAFC] font-sans text-slate-800 overflow-hidden">
      ...
      <div className="w-64 bg-white border-r border-slate-200 flex flex-col shrink-0 z-20">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-1">
            ...
            <div className="mt-6 px-3 text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Pipelines</div>
            {jobs.map(job => (<button key={job.id} onClick={() => setSelectedJob(job)} className={`w-full flex items-center justify-between p-2.5 rounded-lg text-sm font-medium transition ${selectedJob?.id === job.id ? 'bg-indigo-50 text-indigo-700 ring-1 ring-indigo-200' : 'text-slate-600 hover:bg-slate-50'}`}><span className="truncate">{job.title}</span><span className={`text-[10px] px-2 py-0.5 rounded-full ${selectedJob?.id === job.id ? 'bg-white text-indigo-600 shadow-sm' : 'bg-slate-100 text-slate-500'}`}>{job.candidate_count}</span></button>))}
            {showNewJobInput ? <form onSubmit={handleCreateJob} className="mt-2 px-1"><input autoFocus className="w-full p-2 text-sm border rounded" placeholder="Job Title..." value={newJobTitle} onChange={e => setNewJobTitle(e.target.value)} onBlur={() => setShowNewJobInput(false)}/></form> : <button onClick={() => setShowNewJobInput(true)} className="w-full flex items-center gap-2 p-2 text-sm text-slate-400 hover:text-indigo-600 mt-2"><Plus size={16} /> New Pipeline</button>}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        <header className="bg-white border-b border-slate-200 px-8 py-4 flex justify-between items-center shrink-0 z-10">
            <h2 className="text-xl font-bold text-slate-900">{selectedJob ? selectedJob.title : "All Candidates"}</h2>
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50 p-8">
            {viewMode === "list" ? (
                <div className="grid gap-4 grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{filteredProfiles.map(cv => <Card key={cv.id} cv={cv} onClick={() => setSelectedCv(cv)} onDelete={handleDeleteCV} onReprocess={handleReprocess} status={selectedJob ? getStatus(cv) : null} />)}</div>
            ) : (
                <div className="flex gap-6 overflow-x-auto pb-4 h-full">{COLUMNS.map(col => (<div key={col} onDragOver={e => e.preventDefault()} onDrop={e => onDrop(e, col)} className="min-w-[320px] bg-slate-100 rounded-xl flex flex-col h-full border border-slate-200/60"><div className="p-3 border-b border-slate-200/50 bg-slate-50/50 rounded-t-xl flex justify-between font-bold text-xs text-slate-600 uppercase"><span>{col}</span><span className="bg-white px-2 py-0.5 rounded">{filteredProfiles.filter(p => getStatus(p) === col).length}</span></div><div className="flex-1 overflow-y-auto p-3 space-y-2">{filteredProfiles.filter(p => getStatus(p) === col).map(cv => <div key={cv.id} draggable onDragStart={e => onDragStart(e, cv.id)}><Card cv={cv} onClick={() => setSelectedCv(cv)} onDelete={handleDeleteCV} onReprocess={handleReprocess} compact /></div>)}</div></div>))}</div>
            )}
        </div>
...
const Card = ({ cv, onClick, onDelete, onReprocess, status, compact }) => {
  const d = cv.parsed_data || {}
  ...
  return (
    <div onClick={onClick} className={`bg-white p-4 rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-slate-100 hover:shadow-[0_4px_12px_rgba(0,0,0,0.08)] hover:border-indigo-200 transition-all cursor-pointer group relative ${compact ? 'mb-0' : ''}`}>
       ...
       <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-50">
          ...
          {status && <span className="text-[10px] font-bold px-2 py-0.5 bg-slate-100 text-slate-500 rounded-full">{status}</span>}
       </div>
    </div>
  )
}
...
const Drawer = ({ cv, onClose, updateApp, updateProfile, jobs, selectedJobId, assignJob, removeJob }) => {
  const [view, setView] = useState("parsed") // parsed | pdf
  const d = cv.parsed_data || {}
  ...
  // Data Parsing
  // Restore sorting logic for timeline
                    ...
                    <>
                        ...
                        {d.summary && (
                            <section>
                                ...
                                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm text-slate-700 leading-relaxed text-base">
                                    {d.summary}
                                </div>
                            </section>
                        )}
                        ...
                        {skills.length > 0 && (
                            <section>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {skills.map((skill, i) => (
                                        <span key={i} className="px-3 py-1.5 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded-lg text-sm font-semibold shadow-sm">{skill}</span>
                                    ))}
                                </div>
                            </section>
                        )}

                        {/* Experience - RESTORED TIMELINE VIEW */}
                        <section>
                                ...
                                {history.map((job, i) => (
                                    <div key={i} className="relative pl-8">
                                        ...
                                        <div className="relative">
                                            ...
                                            {job.description && (
                                                <p className="mt-3 text-slate-600 text-sm leading-relaxed border-l-2 border-slate-100 pl-3">
                                                    {job.description}
                                                </p>
                                            )}
                                        </div>
                 ...
                 <div className="mt-6 pt-6 border-t border-slate-100 space-y-3">
                    ...
                    {!selectedJobId && (
                        <div className="relative group">
                            <button className="w-full py-2 bg-white border border-indigo-200 text-indigo-600 hover:bg-indigo-50 rounded-lg font-bold text-sm transition">Assign to Job...</button>
                            <div className="absolute bottom-full left-0 right-0 mb-2 bg-white border border-slate-200 shadow-xl rounded-xl p-2 hidden group-hover:block z-50">
                                {jobs.map(j => {
                                    const isAssigned = cv.applications?.some(a => a.job_id === j.id)
                                    return <button key={j.id} onClick={() => !isAssigned && assignJob(cv.id, j.id)} disabled={isAssigned} className="w-full text-left px-3 py-2 text-xs font-bold hover:bg-slate-50 rounded-lg text-slate-700 truncate disabled:opacity-50">{j.title} {isAssigned && "✓"}</button>
                                })}
                            </div>
                        </div>
                    )}
